      ...
      <div class="row">
        ...
        <div class="card">
            <div class="card-header">
              <h3 class="card-title">Liste aller gemeldeten Rezepte</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <div class="row">
              <div class="col-sm-12 col-md-6"></div><div class="col-sm-12 col-md-6"></div></div><div class="row"><div class="col-sm-12"><table id="example2" class="table table-bordered table-hover dataTable dtr-inline" role="grid" aria-describedby="example2_info">
                ...
                <tbody>
                <?php
                    $stmt = $conn->prepare("SELECT * FROM pu_reports WHERE report_type = 2 AND report_status = 1 ORDER BY report_at ASC");
                    $stmt->execute();
                    while($row = $stmt->fetch()) {
                        echo '<tr role="row" class="odd">
                        <td tabindex="0" class="sorting_1">'.$row['report_id'].'</td>
                        <td>'.renderDisplaynameOther($row['report_info']).'</td>
                        ...
                        <td><a href="manageuser.php?id='.$row['report_info'].'">Verwalten</a></td>
                      </tr>';
                    }
                ?>
                </tbody>
