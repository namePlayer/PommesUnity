...
<head>
  ...
  <title>PommesUnity CP | Rezeptverwaltung</title>
      ...
      <div class="row">
        ...
        <div class="card">
            <div class="card-header">
              <h3 class="card-title">Liste aller gesperrten Rezepte</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <div class="row">
              <div class="col-sm-12 col-md-6"></div><div class="col-sm-12 col-md-6"></div></div><div class="row"><div class="col-sm-12"><table id="example2" class="table table-bordered table-hover dataTable dtr-inline" role="grid" aria-describedby="example2_info">
                <thead>
                    <tr role="row"><th class="sorting_asc" rowspan="1" colspan="1">ID</th>
                        ...
                        <th class="sorting" tabindex="0" rowspan="1" colspan="1">Grund</th>
                        <th class="sorting" tabindex="0" rowspan="1" colspan="1">Gemeldet am</th>
                        <th class="sorting" tabindex="0" rowspan="1" colspan="1">Melder</th>
                        <th class="sorting" tabindex="0" rowspan="1" colspan="1">Aktionen</th>
                    </tr>
                </thead>
                <tbody>
                <?php
                    $stmt = $conn->prepare("SELECT * FROM pu_reports WHERE report_type = 1 AND report_status = 1 ORDER BY report_at ASC");
                    $stmt->execute();
                    while($row = $stmt->fetch()) {
                        echo '<tr role="row" class="odd">
                        <td tabindex="0" class="sorting_1">'.$row['report_id'].'</td>
                        <td>'.convertChars(getRecipeName($row['report_info'])).'</td>
                        <td>'.convertChars($row['report_message']).'</td>
                        <td>'.date("d.m.Y", $row['report_at']).' - '.date("H:i", $row['report_at']).'</td>
                        <td>'.renderDisplaynameOther($row['report_by']).'</td>
                        <td><a href="managerecipe.php?id='.$row['report_info'].'">Verwalten</a></td>
                      </tr>';
                    }
                ?>
                </tbody>
